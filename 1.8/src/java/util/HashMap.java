...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * 初始化一个node数组，每一个放着相同hashcode值的链，当链的长度长度超过7时和数组的长度超过，
     * 链就会转化成红黑树
     */
    ...
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {
                ...
                //如果当前数组长度大于初始容量（16），且当前数组长度的2倍小于最大容量时，
                //那么调整因子（threshold）扩大两倍,若果是第一次进resize方法来，threshold = 0 ，所以newThr = 0
